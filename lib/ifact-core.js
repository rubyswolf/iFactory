...
const DOXYGEN_TARGET_OUTPUT_FOLDERS = {
  iplug2: "iPlug2",
  edsp: "eDSP",
  rtneural: "RTNeural",
  retina: "ReTiNA",
...
const normalizeDoxygenTarget = (target) => {
...
const getDoxygenTargetOutputDir = (projectPath, normalizedTarget) =>
  path.join(
    projectPath,
    "doxygen",
    DOXYGEN_TARGET_OUTPUT_FOLDERS[normalizedTarget] || normalizedTarget,
  );
...
const runDoxygenGenerate = async (projectPath, target, options = {}) => {
  ...
  const normalizedTarget = normalizeDoxygenTarget(target);
  if (!normalizedTarget) {
    return { error: "missing_target" };
  }
  const targetConfigs = {
    iplug2: {
      requiredPath: path.join(projectPath, "iPlug2"),
    ...
    edsp: {
      requiredPath: path.join(projectPath, "eDSP"),
      missingError: "edsp_missing",
    ...
    rtneural: {
      requiredPath: path.join(projectPath, "RTNeural"),
      missingError: "rtneural_missing",
      doxyfilePath: path.join(projectPath, "RTNeural", "doxygen", "Doxyfile"),
      cwd: path.join(projectPath, "RTNeural", "doxygen"),
      outputFolder: "RTNeural",
      transformContent: null,
    },
    retina: {
      requiredPath: path.join(projectPath, "ReTiNA"),
      missingError: "retina_missing",
      doxyfilePath: path.join(projectPath, "ReTiNA", "doxygen", "Doxyfile"),
      cwd: path.join(projectPath, "ReTiNA", "doxygen"),
      outputFolder: "ReTiNA",
      transformContent: null,
    },
  };
  ...
  if (config.requiredPath && !fs.existsSync(config.requiredPath)) {
    return { error: config.missingError || "target_missing" };
  }
  ...
  const outputDir = getDoxygenTargetOutputDir(projectPath, normalizedTarget);
